...
- Allow insecure protected store
...
- Fetch position in status is verified
- Initialize fetcher with bootstrap share info, stability
- Tablestore errors, have_map refresh
...
- Add bit iterator to piece_map
- PieceMap::intersection and PieceMap::pop_intersect
- Use remote have-map in fetcher
- [**breaking**] Migrate veilid-core to 0.5.0
- Piece lease manager to distribute pieces to peers

### 🐛 Bug Fixes

- Avoid self-gossip, attempt to remediate status tracking
- Store dht key with secret, fix peer and have map encode
- Havemap intersection operates on a reference
- Use hashset for distributing piece leases
- Share_resolver broken channels
...
- Clean up fetcher, avoid copying index
- Cargo fmt, clippy
- Clean up unused pool tasks, simplify fetch_lease return
...
- Warn on lagged update receiver in seeder, improve logging
- Use explicit owner keypair when writing dht values

### ⚙️ Miscellaneous Tasks

- Debug log connection state try lock, validating a hypothesis
...
- Refine definitions of transient and routing errors
